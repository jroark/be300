...
static probe_region_t g_regions[] = {
    ...
    { "vr4131_bcu_window",         0x0F000000, 0x0020, {FALSE, FALSE, FALSE}, {{0}, {0}, {0}} },
    { "vr4131_icu_pmu_window",     0x0F000080, 0x00A0, {FALSE, FALSE, FALSE}, {{0}, {0}, {0}} }
};
...
static BOOL OpenOutputFile(void)
{
    ...
    for (suffix = 0; suffix < 100; suffix++) {
        if (out_dir[0] == L'\\' && out_dir[1] == L'\0') {
            if (suffix == 0)
                wsprintf(candidate, L"\\BE300Probe_v4_%s_%lu.txt", tag_w, tick);
            else
                wsprintf(candidate, L"\\BE300Probe_v4_%s_%lu_%02d.txt", tag_w, tick, suffix);
        } else {
            if (suffix == 0)
                wsprintf(candidate, L"%s\\BE300Probe_v4_%s_%lu.txt", out_dir, tag_w, tick);
            else
                wsprintf(candidate, L"%s\\BE300Probe_v4_%s_%lu_%02d.txt", out_dir, tag_w, tick, suffix);
        }
...
static BOOL JoinPath(const WCHAR *dir, const WCHAR *name, WCHAR *out_path, DWORD out_cch)
{
    if (!dir || !name || !out_path || out_cch == 0)
        return FALSE;

    if (dir[0] == L'\\' && dir[1] == L'\0')
        wsprintf(out_path, L"\\%s", name);
    else
        wsprintf(out_path, L"%s\\%s", dir, name);
    return TRUE;
}
...
static BOOL SelectFirstRegularRecursive(const WCHAR *current_dir,
                                        int depth,
                                        WCHAR *out_path,
                                        DWORD out_cch,
                                        BOOL *out_direct)
{
    ...
    if (current_dir[0] == L'\\' && current_dir[1] == L'\0')
        CopyWide(search, MAX_PATH, L"\\*");
    else
        wsprintf(search, L"%s\\*", current_dir);
    ...
    do {
        ...
        if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0) {
            JoinPath(current_dir, fd.cFileName, out_path, out_cch);
            if (out_direct)
                *out_direct = (depth == 0);
            FindClose(h);
            return TRUE;
    ...
    do {
        ...
        if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)
            continue;
        JoinPath(current_dir, fd.cFileName, child, MAX_PATH);
        if (SelectFirstRegularRecursive(child, depth + 1, out_path, out_cch, out_direct)) {
            FindClose(h);
            return TRUE;
        }
    } while (FindNextFile(h, &fd));
...
static BOOL SelectFileUnderRoot(const WCHAR *root,
                                WCHAR *out_path,
                                DWORD out_cch,
                                const char **out_found_by)
{
    WCHAR exe_path[MAX_PATH];
    ...
    if (exe_len > 0 && exe_len < MAX_PATH && WideStartsWith(exe_path, root) && FileExistsRegular(exe_path)) {
        CopyWide(out_path, out_cch, exe_path);
        if (out_found_by)
            *out_found_by = "exe_path";
        return TRUE;
    }
    ...
    if (!SelectFirstRegularRecursive(root, 0, out_path, out_cch, &direct))
        return FALSE;

    if (out_found_by)
        *out_found_by = direct ? "direct" : "recursive";
    return TRUE;
}
                                ...
                                const char *found_by)
{
    ...
    Log("[%s] root=\"%s\" selected_file=\"%s\" found_by=%s loops=%d\r\n",
        section_tag, root_a, path_a, found_by ? found_by : "unknown", loops);
    FlushLog();
...
static BOOL RunNandWorkload(void)
{
    ...
    if (!SelectFileUnderRoot(L"\\Nand Disk", path, MAX_PATH, &found_by)) {
        Log("[NAND] status=ABSENT\r\n");
        FlushLog();
        return FALSE;
    }
    return RunFileReadWorkload("NAND", path, NAND_READ_LOOPS, L"\\Nand Disk", found_by);
}
...
static BOOL RunStorageWorkload(void)
{
    ...
    for (i = 0; i < (int)(sizeof(roots) / sizeof(roots[0])); i++) {
        if (!DirectoryExists(roots[i]))
            continue;
        if (SelectFileUnderRoot(roots[i], path, MAX_PATH, &found_by))
            return RunFileReadWorkload("STORAGE", path, STORAGE_READ_LOOPS, roots[i], found_by);
        {
            char root_a[MAX_PATH];
...
static void DumpDirectoryPresence(const WCHAR *root)
{
    char root_a[MAX_PATH];
    WideToAnsi(root, root_a, sizeof(root_a));
    if (!DirectoryExists(root)) {
        Log("[FS_ROOT] path=\"%s\" status=ABSENT\r\n", root_a);
        FlushLog();
        return;
    }
    DumpDirectoryListing(root);
}
...
static void LogRunMetadata(void)
{
    ...
    Log("BE-300 Post-Boot Probe Utility v4\r\n");
    Log("boot_tag=\"%s\"\r\n", g_boot_tag);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    if (!OpenOutputFile()) {
        ...
        MessageBox(NULL, msg, L"BE300 Probe v4", MB_OK | MB_ICONHAND);
        return 1;
    }
    ...
    __try {
        ...
        BeginSection("EARLY SNAPSHOT");
        CaptureSnapshot(SNAP_EARLY);

        Sleep(3000);
        BeginSection("SETTLE SNAPSHOT");
        CaptureSnapshot(SNAP_SETTLE);

        BeginSection("NAND WORKLOAD");
        RunNandWorkload();

        BeginSection("STORAGE WORKLOAD");
        RunStorageWorkload();

        BeginSection("POST SNAPSHOT");
        CaptureSnapshot(SNAP_POST);

        BeginSection("DIFF SUMMARY");
        ...
        BeginSection("DRIVER INVENTORY");
        DumpRegistryTree(L"Drivers\\Active", 0);
        DumpRegistryTree(L"Drivers\\BuiltIn", 0);

        BeginSection("STORAGE MANAGER");
        DumpRegistryTree(L"System\\StorageManager\\Profiles", 0);
        DumpRegistryTree(L"System\\StorageManager\\AutoLoad", 0);

        BeginSection("FILESYSTEM ROOTS");
        DumpDirectoryListing(L"\\");
        DumpDirectoryPresence(L"\\Nand Disk");
        DumpDirectoryPresence(L"\\Storage Card");
        DumpDirectoryPresence(L"\\CF Card");
        DumpDirectoryPresence(L"\\PC Card");
    } __except (EXCEPTION_EXECUTE_HANDLER) {
