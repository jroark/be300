...
static int get_io_irq(socket_info_t *s, int *irq)
{
	return s->ss_entry->get_io_irq(s->sock, irq);
}

static int get_bus_irq(socket_info_t *s, int *irq)
{
	return s->ss_entry->get_bus_irq(s->sock, irq);
}
#endif
...
#ifdef CONFIG_PCMCIA_IRQ_REMAP
			get_bus_irq(s, &irq);
			if ((req->Attributes & IRQ_TYPE) == IRQ_TYPE_DYNAMIC_SHARING) {
			    if (try == 0)
