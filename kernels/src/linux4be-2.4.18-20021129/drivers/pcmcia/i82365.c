...
static int cs_irq = 0;
#ifdef CONFIG_PCMCIA_IRQ_REMAP
/* IRQ map for IO IRQ convert to ISA IRQ */
static int irq_map[16] = { [0 ... 15] = 0 };
#endif
#endif
...
#ifdef CONFIG_PCMCIA_IRQ_REMAP
    probe_irq = probe_irq_off(irqs);
    if (probe_irq) {
	for (i = 0; i < 16; i++)
	    if (irq_map[i] == probe_irq) break;
	if (i >= 16 || i == irq) {
	    irq_map[irq] = probe_irq;
	    irq_hits = 1;
	}
    }
}
...
#define PIC 0x4d0
...
#ifndef CONFIG_PCMCIA_IRQ_REMAP
	...
	if (mask1 & (1<<i))
#ifdef CONFIG_PCMCIA_IRQ_REMAP
	    printk("%s%d", ((mask1 & ((1<<i)-1)) ? "," : ""), irq_map[i]);
#else
...
#ifdef CONFIG_PCMCIA_IRQ_REMAP
static int pcic_get_io_irq(unsigned int sock, int *irq)
{
	int i;

	for (i = 0; i < 16; i++)
		if (irq_map[i] == *irq) break;
...
static int pcic_get_bus_irq(unsigned int sock, int *irq)
{
	if (*irq < 0 || *irq >= 16)
		return -EINVAL;
