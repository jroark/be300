...
lx_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (((count > compare) && (temp > compare))||
	    ((count <= compare) && (temp < compare)) ) { /* none or both rollover */
 		while (temp < count) {
 			(*(unsigned long *)&jiffies)++;
 			temp += sys_timer_reload;
 		}
  	}
	else {
	  if ((count <= compare) && (temp > compare)) {  /* count rollover */ 
	    while (temp > compare ) {
 			(*(unsigned long *)&jiffies)++;
 			temp += sys_timer_reload;
 		}
	...
	/*else if ((count > compare) && (temp < compare),
	     compare rollover, do nothing */
   
 	/* adjust compare */
 	Sys_clock_ctrl_reg[2] = temp;	  
