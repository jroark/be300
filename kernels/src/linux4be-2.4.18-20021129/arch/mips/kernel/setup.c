...
extern unsigned int read_lx_revision(void);
#endif
...
static inline void cpu_probe(void)
{
	...
	case PRID_COMP_LEGACY:
		...
		case PRID_IMP_R3000:
			...
			break;
#ifdef CONFIG_CPU_LX45XXX
		case PRID_IMP_LX4080:
			printk("Detected LX4080 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
			goto Lexra_Next;
		case PRID_IMP_LX4180:
			printk("Detected LX4180 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
			goto Lexra_Next;
		case PRID_IMP_LX4280:
			printk("Detected LX4280 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
			goto Lexra_Next;
		case PRID_IMP_LX4089:
			printk("Detected LX4089 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
			goto Lexra_Next;
		case PRID_IMP_LX4189:
			printk("Detected LX4189 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
			goto Lexra_Next;
		case PRID_IMP_LX5180:
			printk("Detected LX5180 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 1;
			goto Lexra_Next;
		case PRID_IMP_LX5280:
			printk("Detected LX5280 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 1;
			goto Lexra_Next;
		case PRID_IMP_LX8000:
			printk("Detected LX8000 (PRID: %04x), ", 
					mips_cpu.processor_id);
			dspProduct = 0;
Lexra_Next:
		printk("\nRevision: %08x, ", read_lx_revision());
		...
		case PRID_IMP_RC32355:
			mips_cpu.cputype = CPU_RC32300;
			mips_cpu.isa_level = MIPS_CPU_ISA_M32;
			mips_cpu.options = R4K_OPTS | MIPS_CPU_DIVEC |
					   MIPS_CPU_WATCH;
			mips_cpu.tlbsize = 16;
			mips_cpu.icache.ways = 2;
			mips_cpu.dcache.ways = 2;
...
void __init setup_arch(char **cmdline_p)
{
	...
	case MACH_GROUP_EE:
        ...
        case MACH_GROUP_HP_LJ:
                ...
                break;
#endif
#if defined(CONFIG_IDT_79S334) || defined(CONFIG_IDT_79EB355)
	case MACH_GROUP_IDT:
		idt_setup();
		break;
#endif
