...
def collect_window_votes(paths: List[Path], spec: WindowSpec) -> Dict[int, Counter[int]]:
...
def render_header(paths: List[Path]) -> str:
    ...
    required_support = max(2, math.ceil(sample_count * 0.40))
    metadata: Dict[str, Dict[str, object]] = {}
    region_names: List[str] = []
    region_defs: List[str] = []

    for spec in WINDOW_SPECS:
        votes = collect_window_votes(paths, spec)
        words: List[int] = []
        valid: List[int] = []
        stable_words = 0
...
static const uint32_t wince_resume_replay_required_support = UINT32_C({required_support});
