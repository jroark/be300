...
static void be300_reset(void *opaque)
{
    ...
    if (s->kernel_entry != -1ULL) {
        if (s->kernel_cmdline) {
            ...
            /* Restore cmdline memory (in case guest corrupted it) */
...
static void be300_init(MachineState *machine)
{
     ...
     * Base: 0x0f000080
     * Output: Connected to CPU IRQ 2 (typical MIPS primary IRQ)
     */
     ...
     * Base: 0x0f000100
     * Output: Connected to ICU Input 0 (RTCLONG1)
     */
    ...
    if (kernel_filename) {
        ...
        if (kernel_cmdline) {
            ...
            /* Save host-side copy for reset restoration */
