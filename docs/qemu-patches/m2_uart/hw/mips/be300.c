...
/* 
 * 18.432 MHz clock derived from fixed divisor table in serial_be300.c:
 * 115200 bps = clock / (16 * 10) -> clock = 18,432,000 Hz
 */
#define BE300_UART_CLOCK      18432000

/* 
 * CyaCE Shim Handoff: 
 * We place the cmdline and argv table in the first page of RAM.
 */
...
static void be300_init(MachineState *machine)
{
    ...
    DeviceState *icu_dev, *rtc_dev;
    CPUMIPSState *env;
    
    /* CPU Selection with Fallback */
    const char *cpu_type = machine->cpu_type;
    if (!cpu_type) {
        cpu_type = MIPS_CPU_TYPE_NAME("Vr4100");
    }
    ...
    /* 
     * VR4131 ICU (Interrupt Control Unit)
     */
    icu_dev = sysbus_create_simple(TYPE_VR4131_ICU, BE300_VRIP_ICU_ADDR, env->irq[2]);

    /*
     * VR4131 RTC (Real Time Clock)
     */
    rtc_dev = sysbus_create_simple(TYPE_VR4131_RTC, BE300_VRIP_RTC_ADDR, 
                                   qdev_get_gpio_in(icu_dev, 0));

    /*
     * BE-300 UART (Kj Companion Serial)
     * 8250-compatible, 4-byte stride (regshift=2), 18.432 MHz
     */
    if (serial_hd(0)) {
        serial_mm_init(address_space_mem, BE300_UART_BASE, 2,
                       qdev_get_gpio_in(icu_dev, BE300_UART_IRQ),
                       BE300_UART_CLOCK, serial_hd(0),
    ...
    if (kernel_filename) {
        ...
        kernel_size = load_elf(kernel_filename, NULL, cpu_mips_kseg0_to_phys, NULL,
        ...
        if (kernel_cmdline) {
            ...
            if (len >= MAX_CMDLINE_LEN) {
                warn_report("Kernel command line truncated to %d bytes", MAX_CMDLINE_LEN - 1);
                len = MAX_CMDLINE_LEN - 1;
            }
            reset_info->kernel_cmdline = g_strndup(kernel_cmdline, len);
        }
    }
