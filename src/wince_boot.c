    ...
    const unsigned char *data, size_t len, bool is_write)
{
    ...
    /*
     * L2 demand-page marker fixup.
     *
     * When section[0] (PA 0x18C0) is written with a new L2 table
     * pointer, check if that table is missing the demand-page markers
     * (0x00000001) in the DLL VA range.  If so, copy them from the
     * shared/kernel L2 table.
     *
    ...
    if (is_write && paddr == 0x18C0u && len == 4) {
         ...
         * demand-page markers (0x00000001) at the DLL VA range.
         * DLL range L2 offset: (0x01A50000 >> 14) & 0x7FC = 0x694. */
        if (m->wince.shared_l2_table == 0 && new_val != 0
            && new_val != 0x8008BC18u) {
            uint32_t pa = new_val & 0x1FFFFFFFu;
            unsigned char *hp = memory_paddr_to_hostaddr(
                cpu->mem, pa + 0x694u, MEM_READ);
            if (hp) {
                uint32_t marker = hp[0] | (hp[1]<<8)
                    | (hp[2]<<16) | (hp[3]<<24);
                if (marker == 0x00000001u) {
                    m->wince.shared_l2_table = new_val;
                    fprintf(stderr,
                        "[L2_FIXUP] shared L2 table = 0x%08X"
                        " (has demand-page markers)\n",
                        new_val);
                }
            }
        }

        /* If we know the shared table, fix any different table that
         * is missing demand-page markers in the DLL range */
        if (m->wince.shared_l2_table != 0
            && new_val != m->wince.shared_l2_table
            && new_val != 0 && new_val != 0x8008BC18u) {
            uint32_t proc_pa = new_val & 0x1FFFFFFFu;
            bool already_fixed = false;
            int fi;
            for (fi = 0; fi < m->wince.l2_fixup_count; fi++) {
                if (m->wince.l2_fixup_applied[fi] == proc_pa) {
                    already_fixed = true;
                    break;
                }
            }
            if (!already_fixed) {
                /* Copy demand-page markers from shared → process L2.
                 * DLL VA range: 0x01800000-0x01FFFFFF
                 *   L2 byte offsets: (0x01800000>>14)&0x7FC = 0x600
                 *                    to (0x01FFC000>>14)&0x7FC = 0x7FC
                 * Copy entries 0x600-0x7FC (128 entries, 512 bytes). */
                uint32_t shared_pa = m->wince.shared_l2_table
                    & 0x1FFFFFFFu;
                unsigned off;
                int copied = 0;
                for (off = 0x600u; off <= 0x7FCu; off += 4u) {
                    unsigned char *sp = memory_paddr_to_hostaddr(
                        cpu->mem, shared_pa + off, MEM_READ);
                    unsigned char *pp = memory_paddr_to_hostaddr(
                        cpu->mem, proc_pa + off, MEM_READ);
                    if (!sp || !pp)
                        continue;
                    uint32_t sv = sp[0] | (sp[1]<<8)
                        | (sp[2]<<16) | (sp[3]<<24);
                    uint32_t pv = pp[0] | (pp[1]<<8)
                        | (pp[2]<<16) | (pp[3]<<24);
                    if (sv != 0 && pv == 0) {
                        pp[0] = sp[0]; pp[1] = sp[1];
                        pp[2] = sp[2]; pp[3] = sp[3];
                ...
                if (m->wince.l2_fixup_count < 32)
                    m->wince.l2_fixup_applied[
                        m->wince.l2_fixup_count++] = proc_pa;
                if (copied > 0)
                    fprintf(stderr,
                        "[L2_FIXUP] proc table 0x%08X"
                        " (PA 0x%08X): copied %d"
                        " demand-page markers from"
                        " shared 0x%08X\n",
                        new_val, proc_pa, copied,
                        m->wince.shared_l2_table);
            }
        }
    }
}
