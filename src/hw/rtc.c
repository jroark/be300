...
uint32_t rtc_read(rtc_state_t *s, uint32_t offset, unsigned size)
{
    (void)size;
    switch (offset) {
    case RTC_ETIMELREG: return  (uint32_t)(s->etime        & 0xFFFF);
    case RTC_ETIMEMREG: return  (uint32_t)((s->etime >> 16) & 0xFFFF);
    case RTC_ETIMEHREG: return  (uint32_t)((s->etime >> 32) & 0xFFFF);
